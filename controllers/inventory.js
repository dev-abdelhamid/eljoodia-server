...
const updateStock = async (req, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const { currentStock, minStockLevel, maxStockLevel, productId, branchId, orderId } = req.body;
...
const createRestockRequest = async (req, res) => {
  ...
  try {
    ...
    const { productId, branchId, requestedQuantity, notes, orderId } = req.body;
...
const approveRestockRequest = async (req, res) => {
  ...
  try {
    ...
    const { approvedQuantity, userId, orderId } = req.body;
    ...
    restockRequest.order = orderId || restockRequest.order;
    await restockRequest.save({ session });
      ...
      { product: restockRequest.product, branch: restockRequest.branch },
      {
        ...
        $push: {
          movements: {
            ...
            createdBy: userId,
            createdAt: new Date(),
            order: orderId || restockRequest.order || null,
          },
        },
      },
      { upsert: true, new: true, session }
    ...
    req.io?.emit('restockApproved', {
      ...
      quantity: approvedQuantity,
      orderId: orderId || restockRequest.order || null,
    });
    req.io?.emit('inventoryUpdated', {
      branchId: restockRequest.branch.toString(),
      productId: restockRequest.product.toString(),
...
const getInventoryHistory = async (req, res) => {
  try {
    const { branchId, productId, orderId } = req.query;
    const query = {};
...
const createReturn = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      if (!isValidObjectId(item.productId) || item.quantity < 1 || !['تالف', 'منتج خاطئ', 'كمية زائدة', 'أخرى'].includes(item.reason)) {
        await session.abortTransaction();
    ...
    for (const item of items) {
      ...
      if (!inventoryUpdate) {
        await session.abortTransaction();
        return res.status(400).json({ success: false, message: `المخزون غير موجود للمنتج ${item.productId}` });
      }
    ...
    }).select('_id role').session(session);
...
const processReturnItems = async (req, res) => {
  ...
  try {
    ...
    const returnRequest = await Return.findById(returnId).populate('order').session(session);
    if (!returnRequest) {
      await session.abortTransaction();
    ...
    if (req.user.role !== 'admin' && req.user.role !== 'production') {
      await session.abortTransaction();
      return res.status(403).json({ success: false, message: 'غير مخول لمعالجة الإرجاع' });
    }
    ...
    for (const item of items) {
      if (!isValidObjectId(item.productId) || !['approved', 'rejected'].includes(item.status)) {
        await session.abortTransaction();
      ...
      if (!returnItem) {
        await session.abortTransaction();
        return res.status(400).json({ success: false, message: `العنصر ${item.productId} غير موجود في طلب الإرجاع` });
      }
    }
    ...
    returnRequest.statusHistory.push({
      ...
      notes: items.map(i => i.reviewNotes).join('; '),
      changedAt: new Date(),
    });
    await returnRequest.save({ session });
    ...
    await notifyUsers(
      ...
      'notifications.return_status_updated',
      {
        ...
        branchId,
        status: returnRequest.status,
        eventId: `${returnId}-return_status_updated`,
      }
    );
