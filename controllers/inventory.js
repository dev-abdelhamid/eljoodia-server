...
const createInventory = async (req, res) => {
  ...
  try {
    ...
    const { branchId, product, userId, currentStock, minStockLevel = 10, maxStockLevel = 100, orderId } = req.body;

    // Validate inputs
    if (!isValidObjectId(branchId) || !isValidObjectId(product) || !isValidObjectId(userId) || currentStock < 0) {
      console.log(`[${new Date().toISOString()}] إنشاء عنصر مخزون - بيانات غير صالحة:`, { branchId, product, userId, currentStock });
    ...
    const [productDoc, branch] = await Promise.all([
      Product.findById(product).session(session),
      Branch.findById(branchId).session(session),
    ]);
    if (!productDoc) {
      console.log(`[${new Date().toISOString()}] إنشاء عنصر مخزون - المنتج غير موجود:`, { product });
    ...
    if (inventory.currentStock <= inventory.minStockLevel) {
      req.io?.emit('lowStockWarning', {
        branchId,
        product,
        productName: translateField(productDoc, 'name', lang),
...
const bulkCreate = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      const { product, currentStock, minStockLevel = 10, maxStockLevel = 100 } = item;

      if (!isValidObjectId(product) || currentStock < 0) {
        console.log(`[${new Date().toISOString()}] إنشاء دفعة مخزون - بيانات عنصر غير صالحة:`, { product, currentStock });
        throw new Error(isRtl ? `بيانات غير صالحة للمنتج ${product}` : `Invalid data for product ${product}`);
      }

      const productDoc = products.find((p) => p._id.toString() === product);
      if (!productDoc) {
        console.log(`[${new Date().toISOString()}] إنشاء دفعة مخزون - المنتج غير موجود:`, { product });
        throw new Error(isRtl ? `المنتج ${product} غير موجود` : `Product ${product} not found`);
      }
...
const getInventoryByBranch = async (req, res) => {
  ...
  try {
    ...
    // Build query
    const query = { branch: branchId, product: { $ne: null } }; // Filter out null products
...
const getInventory = async (req, res) => {
  ...
  try {
    ...
    // Build query
    const query = { product: { $ne: null } }; // Filter out null products
