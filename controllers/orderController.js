...
const createReturn = async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      if (
        !isValidObjectId(item.itemId) ||
        !isValidObjectId(item.product) ||
        !item.quantity ||
        !['defective', 'wrong_item', 'not_needed', 'other'].includes(item.reason)
      ) {
        await session.abortTransaction();
      ...
      if (!orderItem || orderItem.product._id.toString() !== item.product.toString()) {
        await session.abortTransaction();
        console.error(`[${new Date().toISOString()}] Order item not found or product mismatch:`, {
          itemId: item.itemId,
          product: item.product,
          userId: req.user.id,
        });
    ...
    const notificationMessage = `تم إنشاء طلب إرجاع جديد ${orderNumber} للطلب ${order.orderNumber}`;
    for (const user of usersToNotify) {
      await createNotification(
        user._id,
...
const approveReturn = async (req, res) => {
  ...
  try {
    ...
    const notificationMessage = `تم ${status === 'approved' ? 'الموافقة' : 'الرفض'} على طلب الإرجاع ${returnRequest.orderNumber} للطلب ${returnRequest.order?.orderNumber || 'Unknown'}`;
    for (const user of usersToNotify) {
      await createNotification(
        user._id,
        'return_status_updated',
...
const startTransit = async (req, res) => {
  ...
  try {
    ...
    if (order.status !== 'completed') {
      await session.abortTransaction();
      console.error(`[${new Date().toISOString()}] Invalid order status for starting transit: ${order.status}, User: ${req.user.id}`);
      return res.status(400).json({ success: false, message: 'يجب أن يكون الطلب في حالة "مكتمل" لبدء النقل' });
    }

    if (req.user.role !== 'admin' && req.user.role !== 'production') {
      await session.abortTransaction();
      console.error(`[${new Date().toISOString()}] Unauthorized transit start attempt:`, { userId: req.user.id, role: req.user.role });
      return res.status(403).json({ success: false, message: 'غير مخول لبدء النقل' });
    }

    const tasks = await ProductionAssignment.find({ order: id }).session(session);
    if (!tasks.every(task => task.status === 'completed')) {
      await session.abortTransaction();
      console.error(`[${new Date().toISOString()}] Not all tasks completed for order: ${id}, User: ${req.user.id}`);
      return res.status(400).json({ success: false, message: 'يجب إكمال جميع المهام قبل بدء النقل' });
    }
    ...
    for (const user of usersToNotify) {
      await createNotification(
        ...
        `الطلب ${order.orderNumber} في الطريق إلى ${populatedOrder.branch?.name || 'Unknown'}`,
...
const confirmDelivery = async (req, res) => {
  ...
  try {
    ...
    if (req.user.role !== 'branch' || order.branch?.toString() !== req.user.branchId.toString()) {
      await session.abortTransaction();
      console.error(`[${new Date().toISOString()}] Unauthorized delivery confirmation:`, {
        userId: req.user.id,
        role: req.user.role,
        userBranch: req.user.branchId,
        orderBranch: order.branch,
    ...
    const notificationMessage = `تم تسليم الطلب ${order.orderNumber} إلى ${populatedOrder.branch?.name || 'Unknown'}`;
    for (const user of usersToNotify) {
      await createNotification(
        user._id,
        'order_delivered',
...
const updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    if (!validateStatusTransition((await Order.findById(id).select('status').lean())?.status, status)) {
      await session.abortTransaction();
      console.error(`[${new Date().toISOString()}] Invalid status transition:`, { currentStatus: (await Order.findById(id).select('status').lean())?.status, newStatus: status, userId: req.user.id });
      return res.status(400).json({ success: false, message: 'انتقال حالة غير صالح' });
    }
    ...
    if (status === 'completed') {
      ...
      if (!tasks.every(task => task.status === 'completed')) {
        await session.abortTransaction();
        console.error(`[${new Date().toISOString()}] Not all tasks completed for order: ${id}, User: ${req.user.id}`);
        return res.status(400).json({ success: false, message: 'يجب إكمال جميع المهام قبل إكمال الطلب' });
      }
    }
    ...
    const notificationMessage = `تم تحديث حالة الطلب ${order.orderNumber} إلى "${status}"`;
    for (const user of usersToNotify) {
      await createNotification(
        user._id,
        'order_status_updated',
        notificationMessage,
        { orderId: id, orderNumber: order.orderNumber, branchId: order.branch, eventId: `${id}-order_status_updated` },
        io
      );
    }
