...
const { createNotification, emitSocketEvent, notifyUsers } = require('../utils/notifications');
...
const createOrder = async (req, res) => {
  try {
    ...
    await newOrder.save();
    await syncOrderTasks(newOrder._id, req.app.get('io'));
    ...
    await notifyUsers(
      ...
      users,
      'orderCreated',
      `طلب جديد ${orderNumber} من ${populatedOrder.branch?.name || 'غير معروف'}`,
      eventData,
      false
    );
    const orderData = {
