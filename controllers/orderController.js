...
const assignChefs = async (req, res) => {
  ...
  try {
    ...
    const chefs = await User.find({ _id: { $in: chefIds }, role: 'chef' }).select('name department').lean();
    const chefMap = new Map(chefs.map(c => [c._id.toString(), c]));

    const assignments = [];
    const chefNotifications = [];
    const productDeptMap = new Map(order.items.map(i => [i._id.toString(), i.product.department.toString()]));
    ...
    for (const item of items) {
      ...
      if (!chef) {
        throw new Error(`الشيف ${item.assignedTo} غير موجود أو ليس شيفًا`);
      }

      const productDept = productDeptMap.get(itemId);
      if (productDept && chef.department.toString() !== productDept) {
        throw new Error(`الشيف ${chef.name} لا يمكنه التعامل مع قسم هذا المنتج`);
      }

      const existingTask = await mongoose.model('ProductionAssignment').findOne({ order: orderId, itemId }).session(session).lean();
      ...
      await Order.updateOne(
        ...
        { $set: { 'items.$.assignedTo': item.assignedTo, 'items.$.status': 'assigned' } },
        { session }
      );
    ...
    const populatedOrder = await Order.findById(orderId)
      ...
      .populate('items.assignedTo', 'name') // استرجاع حقل name بدلاً من username
...
const startTransit = async (req, res) => {
  ...
  try {
    ...
    order.statusHistory.push({
      ...
      changedBy: req.user.id,
      changedAt: new Date(),
      notes: 'بدء الشحن بواسطة قسم الإنتاج',
    });
...
const updateOrderStatus = async (req, res) => {
  ...
  try {
    ...
    if (!['admin', 'production'].includes(req.user.role) && (req.user.role !== 'branch' || order.branch.toString() !== req.user.branchId.toString())) {
      await session.abortTransaction();
    ...
    order.statusHistory.push({
      ...
      changedBy: req.user.id,
      changedAt: new Date(),
      notes: `تم تحديث الحالة إلى ${status}`,
    });
...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    const products = await mongoose.model('Product').find({ _id: { $in: mergedItems.map(i => i.product) } }).select('price').lean();
    ...
    const newOrder = new Order({
      ...
      items: mergedItems.map(item => ({
        ...
        quantity: item.quantity,
        price: productPriceMap.get(item.product.toString()) || item.price,
        status: 'pending',
      })),
      status,
      notes: notes?.trim(),
      ...
      totalAmount: mergedItems.reduce((sum, item) => sum + item.quantity * (productPriceMap.get(item.product.toString()) || item.price), 0),
      adjustedTotal: mergedItems.reduce((sum, item) => sum + item.quantity * (productPriceMap.get(item.product.toString()) || item.price), 0),
