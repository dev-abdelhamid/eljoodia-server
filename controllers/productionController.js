...
const createTask = async (req, res) => {
  ...
  try {
    ...
    await Promise.all([
      emitSocketEvent(io, [`chef-${chefProfile._id}`, 'admin', 'production', `branch-${orderDoc.branch}`], 'taskAssigned', taskAssignedEvent),
      createNotification(
        chef,
        'task_assigned',
  ...
  try {
    ...
    if (status === 'in_progress' && order.status === 'approved') {
      ...
      await Promise.all([
        ...
        emitSocketEvent(io, [`branch-${order.branch}`, 'admin', 'production'], 'orderStatusUpdated', orderStatusUpdatedEvent)
      ]);
    }
    ...
    if (status === 'completed') {
      ...
      await Promise.all([
        emitSocketEvent(io, [`chef-${task.chef}`, `branch-${order.branch}`, 'admin', 'production'], 'taskCompleted', taskCompletedEvent),
        createNotification(
          task.chef._id,
          'task_completed',
