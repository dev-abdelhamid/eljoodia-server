...
const assignChefs = async (req, res) => {
  ...
  try {
    ...
    if (!isValidObjectId(orderId) || !Array.isArray(items) || items.length === 0) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: isRtl ? 'معرف الطلب أو مصفوفة العناصر مطلوبة' : 'Order ID and items array are required' });
    }

    const order = await Order.findById(orderId)
      .populate({
        path: 'items.product',
        populate: { path: 'department', select: 'name nameEn code _id' }
      })
    ...
    if (!['approved', 'in_production'].includes(order.status)) {
      await session.abortTransaction();
      return res.status(400).json({ success: false, message: isRtl ? 'الطلب يجب أن يكون معتمد أو قيد الإنتاج' : 'Order must be approved or in production' });
    }

    // جلب الشيفات + ملفاتهم
    const chefUserIds = [...new Set(items.map(i => i.assignedTo).filter(isValidObjectId))];
    const chefs = await User.find({ _id: { $in: chefUserIds }, role: 'chef' }).lean();
    const chefProfiles = await mongoose.model('Chef').find({ user: { $in: chefUserIds } }).lean();

    const chefMap = new Map(chefs.map(c => [c._id.toString(), c]));
    const chefProfileMap = new Map(chefProfiles.map(p => [p.user.toString(), p]));

    // بناء خريطة: userId → [deptId1, deptId2]
    const chefDeptMap = {};
    chefProfiles.forEach(profile => {
      const userId = profile.user.toString();
      const depts = Array.isArray(profile.department)
        ? profile.department.map(d => d.toString())
        : profile.department ? [profile.department.toString()] : [];
      chefDeptMap[userId] = depts;
    });
    ...
    for (const item of items) {
      ...
      if (!isValidObjectId(itemId) || !isValidObjectId(item.assignedTo)) {
        throw new Error(isRtl ? `معرفات غير صالحة` : `Invalid IDs`);
      }

      const orderItem = order.items.find(i => i._id.toString() === itemId);
      if (!orderItem) throw new Error(isRtl ? `العنصر غير موجود` : `Item not found`);

      const productDeptId = orderItem.product.department._id.toString();
      const allowedDepts = chefDeptMap[item.assignedTo] || [];

      // التحقق الأمني: هل الشيف مسموح له بالقسم؟
      if (!allowedDepts.includes(productDeptId)) {
        ...
        throw new Error(isRtl
          ? `الشيف غير مسموح له بالعمل في قسم "${deptName}"`
          : `Chef not allowed in department "${deptName}"`
        );
      }

      const chef = chefMap.get(item.assignedTo);
      const chefProfile = chefProfileMap.get(item.assignedTo);
      if (!chef || !chefProfile) throw new Error(isRtl ? 'الشيف غير صالح' : 'Invalid chef');
      ...
      chefNotifications.push({
        ...
        data: {
          ...
          branchName: isRtl ? order.branch.name : (order.branch.nameEn || order.branch.name),
          taskId: itemId,
    ...
    await notifyUsers(io, [...adminUsers, ...productionUsers, ...branchUsers], 'taskAssigned', '', taskAssignedEventData, false, isRtl);

    for (const notif of chefNotifications) {
      await notifyUsers(io, [{ _id: notif.userId }], 'taskAssigned', notif.message, notif.data, false, isRtl);
    }

    const rooms = new Set(['admin', 'production', `branch-${order.branch._id}`]);
    chefUserIds.forEach(id => rooms.add(`chef-${id}`));
    await emitSocketEvent(io, [...rooms], 'taskAssigned', taskAssignedEventData, isRtl);
    ...
    const populatedOrder = await Order.findById(orderId)
      ...
      .populate('items.assignedTo', 'name nameEn')
      .lean();
    ...
    res.json({
      ...
      order: {
        ...
        items: populatedOrder.items.map(item => ({
          ...
          assignedToName: isRtl ? item.assignedTo?.name : (item.assignedTo?.nameEn || item.assignedTo?.name),
        })),
      }
    });
    ...
    await session.abortTransaction();
    console.error('assignChefs error:', err.message);
