...
apiNamespace.on('connection', (socket) => {
  ...
  socket.on('orderCreated', (data) => {
    const eventData = { ...data, sound: 'https://eljoodia.vercel.app/sounds/notification.mp3', vibrate: [300, 100, 300] };
  ...
  socket.on('orderApproved', (data) => {
    const eventData = { ...data, sound: 'https://eljoodia.vercel.app/sounds/notification.mp3', vibrate: [200, 100, 200] };
  ...
  socket.on('taskAssigned', (data) => {
    const eventData = { ...data, sound: 'https://eljoodia.vercel.app/sounds/notification.mp3', vibrate: [400, 100, 400] };
    apiNamespace.to('admin').emit('taskAssigned', eventData);
    apiNamespace.to('production').emit('taskAssigned', eventData);
    if (data.chef) apiNamespace.to(`chef-${data.chef._id}`).emit('taskAssigned', eventData);
  ...
  socket.on('taskStatusUpdated', ({ taskId, status, orderId, itemId, branchId }) => {
    const eventData = { taskId, status, orderId, itemId, branchId, sound: 'https://eljoodia.vercel.app/sounds/notification.mp3', vibrate: [200, 100, 200] };
    apiNamespace.to('admin').emit('taskStatusUpdated', eventData);
    apiNamespace.to('production').emit('taskStatusUpdated', eventData);
    if (branchId) apiNamespace.to(`branch-${branchId}`).emit('taskStatusUpdated', eventData);
  });
  ...
  socket.on('orderStatusUpdated', async ({ orderId, status, user, branchId }) => {
    const eventData = { orderId, status, user, branchId, sound: 'https://eljoodia.vercel.app/sounds/notification.mp3', vibrate: [200, 100, 200] };
    apiNamespace.to('admin').emit('orderStatusUpdated', eventData);
    apiNamespace.to('production').emit('orderStatusUpdated', eventData);
    if (branchId) apiNamespace.to(`branch-${branchId}`).emit('orderStatusUpdated', eventData);
    ...
    if (status === 'completed' && order) {
      ...
      if (order.branch) apiNamespace.to(`branch-${order.branch._id}`).emit('orderCompleted', completedEventData);
      const usersToNotify = await User.find({ role: { $in: ['branch', 'admin', 'production'] }, branchId: order.branch._id }).select('_id').lean();
      await createNotification(null, 'order_completed',
    ...
    if (status === 'in_transit' && order) {
      ...
      if (order.branch) apiNamespace.to(`branch-${order.branch._id}`).emit('orderInTransit', transitEventData);
      const usersToNotify = await User.find({ role: { $in: ['branch', 'admin', 'production'] }, branchId: order.branch._id }).select('_id').lean();
      await createNotification(null, 'order_in_transit',
        `الطلب ${order.orderNumber} في الطريق`,
        { orderId, orderNumber: order.orderNumber, branchId: order.branch._id },
        io
      );
    }
    if (status === 'delivered' && order) {
      ...
      if (order.branch) apiNamespace.to(`branch-${order.branch._id}`).emit('orderDelivered', deliveredEventData);
      const usersToNotify = await User.find({ role: { $in: ['branch', 'admin', 'production'] }, branchId: order.branch._id }).select('_id').lean();
      await createNotification(null, 'order_delivered',
        `تم تسليم الطلب ${order.orderNumber}`,
        { orderId, orderNumber: order.orderNumber, branchId: order.branch._id },
        io
      );
    }
  ...
  socket.on('returnStatusUpdated', ({ returnId, status, returnNote, branchId }) => {
    const eventData = { returnId, status, returnNote, branchId, sound: 'https://eljoodia.vercel.app/sounds/notification.mp3', vibrate: [200, 100, 200] };
    apiNamespace.to('admin').emit('returnStatusUpdated', eventData);
    apiNamespace.to('production').emit('returnStatusUpdated', eventData);
    if (branchId) apiNamespace.to(`branch-${branchId}`).emit('returnStatusUpdated', eventData);
  });
  ...
  socket.on('missingAssignments', (data) => {
    ...
    if (data.branchId) apiNamespace.to(`branch-${data.branchId}`).emit('missingAssignments', eventData);
  });
