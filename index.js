...
const productionAssignmentRoutes = require('./routes/productionAssignments');
...
io.on('connection', (socket) => {
  ...
  socket.on('taskCompleted', (data) => {
    ...
    if (data.orderId) {
      Order.findById(data.orderId).then((order) => {
        if (order?.branch) {
  ...
  socket.on('returnStatusUpdated', ({ returnId, status, returnNote }) => {
    ...
    Return.findById(returnId).then((returnRequest) => {
