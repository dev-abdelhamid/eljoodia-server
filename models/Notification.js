...
const setupNotifications = (io, socket) => {
  ...
  const handleTaskAssigned = async (data) => {
    const { orderId, taskId, chefId, productId, productName, quantity, branchId, departmentId } = data;
    const order = await Order.findById(orderId).populate('branch', 'name').lean();
    if (!order) return;

    const rooms = new Set(['admin', 'production', `chef-${chefId}`, `branch-${order.branch?._id || branchId}`, `department-${departmentId}`]);
    const eventData = {
      ...
      data: { orderId, taskId, branchId: order.branch?._id || branchId, chefId, productId, productName, quantity, departmentId, eventId: `${taskId}-task_assigned` },
      read: false,
      createdAt: new Date().toISOString(),
  ...
  const handleTaskCompleted = async (data) => {
    const { orderId, taskId, chefId, productName, branchId, departmentId } = data;
    const order = await Order.findById(orderId).populate('branch', 'name').lean();
    if (!order) return;
    ...
    const eventData = {
      ...
      data: { orderId, taskId, branchId: order.branch?._id || branchId, chefId, departmentId, eventId: `${taskId}-task_completed` },
      read: false,
      createdAt: new Date().toISOString(),
  ...
  const handleInventoryUpdated = async (data) => {
    const { branchId, productId, currentStock } = data;
    const rooms = new Set(['admin', 'production', `branch-${branchId}`]);
    const eventData = {
      _id: `${productId}-inventoryUpdated-${Date.now()}`,
      type: 'inventory_updated',
      message: `تم تحديث المخزون للمنتج (المخزون الحالي: ${currentStock})`,
      data: { branchId, productId, currentStock, eventId: `${productId}-inventory_updated` },
      read: false,
      createdAt: new Date().toISOString(),
      sound: 'https://eljoodia-client.vercel.app/sounds/inventory_updated.mp3',
      vibrate: [200, 100, 200],
      timestamp: new Date().toISOString(),
    };

    rooms.forEach(room => io.to(room).emit('newNotification', eventData));
    console.log(`[${new Date().toISOString()}] Emitted newNotification for inventoryUpdated to rooms: ${[...rooms].join(', ')}`);
    ...
    for (const user of [...adminUsers, ...productionUsers, ...branchUsers]) {
      await createNotification(user._id, 'inventory_updated', eventData.message, eventData.data, io);
    }
  };
