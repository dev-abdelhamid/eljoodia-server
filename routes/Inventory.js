...
router.post(
  ...
  [
    auth,
    authorize(['branch', 'admin']),
    body('branchId').notEmpty().custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف الفرع غير صالح'),
    body('productId').notEmpty().custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف المنتج غير صالح'),
    body('userId').notEmpty().custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف المستخدم غير صالح'),
...
router.post(
  ...
  [
    ...
    body('notes').optional().trim().isLength({ max: 500 }).withMessage('الملاحظات يجب ألا تتجاوز 500 حرف'),
  ],
  createRestockRequest
);
...
router.patch(
  ...
  [
    auth,
    authorize(['admin']),
    param('requestId').custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف طلب إعادة التخزين غير صالح'),
...
router.post(
  ...
  [
    auth,
    authorize(['branch']),
    body('orderId').notEmpty().custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف الطلب غير صالح'),
    body('branchId').notEmpty().custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف الفرع غير صالح'),
    body('reason').notEmpty().trim().isLength({ min: 3, max: 500 }).withMessage('سبب الإرجاع يجب أن يكون بين 3 و500 حرف'),
    body('items').isArray({ min: 1 }).withMessage('يجب أن تحتوي العناصر على عنصر واحد على الأقل'),
    body('items.*.productId').notEmpty().custom((value) => mongoose.isValidObjectId(value)).withMessage('معرف المنتج غير صالح'),
    body('items.*.quantity').isInt({ min: 1 }).withMessage('الكمية يجب أن تكون أكبر من 0'),
    body('items.*.reason').notEmpty().trim().isLength({ min: 3, max: 500 }).withMessage('سبب الإرجاع للعنصر يجب أن يكون بين 3 و500 حرف'),
    body('notes').optional().trim().isLength({ max: 500 }).withMessage('الملاحظات يجب ألا تتجاوز 500 حرف'),
  ],
  createReturn
);
...
router.patch(
  ...
  [
    ...
    body('items.*.reviewNotes').optional().trim().isLength({ max: 500 }).withMessage('ملاحظات المراجعة يجب ألا تتجاوز 500 حرف'),
  ],
  processReturnItems
);
