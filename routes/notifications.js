...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(`[${new Date().toISOString()}] Creating notification for user ${userId}:`, { type, message, data, recipients });

      // إنشاء الإشعار لكل مستلم أو المستخدم الحالي إذا لم يتم تحديد مستلمين
      const notifications = [];
      const targetUsers = recipients.length > 0 ? recipients : [userId];
      for (const targetUserId of targetUsers) {
        const notification = await createNotification(targetUserId, type, message, data, req.app.get('io'));
