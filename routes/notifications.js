...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        console.error(`[${new Date().toISOString()}] Validation errors in POST /notifications:`, {
          errors: errors.array(),
          userId: req.user.id,
        });
      ...
      if (existingNotification) {
        console.warn(`[${new Date().toISOString()}] Duplicate notification blocked:`, { eventId, userId });
        return res.status(200).json({ success: true, data: existingNotification });
      }

      console.log(`[${new Date().toISOString()}] Creating notification for user ${userId}:`, { type, message, data });

      const notification = await createNotification(userId, type, message, { ...data, eventId }, req.app.get('io'));
...
router.patch('/:id/read', [auth, notificationLimiter], async (req, res) => {
  try {
    ...
    if (!notification) {
      console.error(`[${new Date().toISOString()}] Notification not found or unauthorized:`, {
        id: req.params.id,
        userId: req.user.id,
      });
    ...
    console.error(`[${new Date().toISOString()}] Error in PATCH /notifications/:id/read:`, {
      error: err.message,
      userId: req.user.id,
      stack: err.stack,
    });
...
router.patch('/mark-all-read', [auth, notificationLimiter], async (req, res) => {
  try {
    ...
    console.error(`[${new Date().toISOString()}] Error in PATCH /notifications/mark-all-read:`, {
      error: err.message,
      userId: req.user.id,
      stack: err.stack,
    });
...
router.delete('/:id', [auth, notificationLimiter], async (req, res) => {
  try {
    ...
    console.error(`[${new Date().toISOString()}] Error in DELETE /notifications/:id:`, {
      error: err.message,
      userId: req.user.id,
      stack: err.stack,
    });
