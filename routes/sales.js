...
const buildQuery = (params, user, isBranchAnalytics = false) => {
  const query = {};
  const { branch, department, startDate, endDate, search } = params;

  if (branch && isValidObjectId(branch)) {
    query.branch = new mongoose.Types.ObjectId(branch);
  } else if (isBranchAnalytics && user.role === 'branch' && user.branchId) {
    query.branch = new mongoose.Types.ObjectId(user.branchId);
  }

  if (department && isValidObjectId(department)) {
    query['items.product'] = {
      $in: Product.find({ department: new mongoose.Types.ObjectId(department) }).distinct('_id'),
    };
  }
...
// Helper function for analytics aggregations
const getAnalyticsAggregations = async (query, isRtl, dateFormat) => {
  ...
  const productSales = await Sale.aggregate([
    ...
    },
    {
      $lookup: {
        ...
        foreignField: '_id',
        as: 'product',
        pipeline: [{ $project: { name: 1, nameEn: 1, department: 1 } }],
      },
    },
...
router.get(
  ...
  [
    ...
    query('sort').optional().isString().withMessage('الترتيب غير صالح'),
    query('lang').optional().isIn(['ar', 'en']).withMessage('اللغة غير صالحة'),
  ],
  async (req, res) => {
    const { branch, startDate, endDate, page = 1, limit = 20, sort = '-createdAt', lang = 'ar', search } = req.query;
    const isRtl = lang === 'ar';
    try {
      const query = buildQuery({ branch, startDate, endDate, search }, req.user);
      ...
      console.log(`[${new Date().toISOString()}] Get sales - Success:`, { total, page, limit, query });
...
router.get(
  ...
  [
    ...
    query('startDate').optional().isISO8601().toDate().withMessage('تاريخ البداية غير صالح'),
    query('endDate').optional().isISO8601().toDate().withMessage('تاريخ النهاية غير صالح'),
    query('search').optional().isString().trim().withMessage('البحث يجب أن يكون نصًا'),
    query('lang').optional().isIn(['ar', 'en']).withMessage('اللغة غير صالحة'),
  ],
  async (req, res) => {
    try {
      await mongoose.connection; // Ensure DB connection
      const { branch, department, startDate, endDate, search, lang = 'ar' } = req.query;
      const isRtl = lang === 'ar';

      if ((branch && !isValidObjectId(branch)) || (department && !isValidObjectId(department))) {
        return res.status(400).json({ success: false, message: isRtl ? 'معرف فرع أو قسم غير صالح' : 'Invalid branch or department ID' });
      }

      const query = await buildQuery({ branch, department, startDate, endDate, search }, req.user);
      const dateFormat = startDate && endDate && (new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24) > 30 ? 'month' : 'day';

      const analytics = await getAnalyticsAggregations(query, isRtl, dateFormat);
...
router.get(
  ...
  async (req, res) => {
    try {
      await mongoose.connection; // Ensure DB connection
      const { department, startDate, endDate, search, lang = 'ar' } = req.query;
      const isRtl = lang === 'ar';
      ...
      if (department && !isValidObjectId(department)) {
        return res.status(400).json({ success: false, message: isRtl ? 'معرف القسم غير صالح' : 'Invalid department ID' });
      }

      const query = await buildQuery({ department, startDate, endDate, search }, req.user, true);
      const dateFormat = startDate && endDate && (new Date(endDate) - new Date(startDate)) / (1000 * 60 * 60 * 24) > 30 ? 'month' : 'day';
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!isValidObjectId(id)) {
        console.error(`[${new Date().toISOString()}] Get sale - Invalid ID:`, { id });
        return res.status(400).json({ success: false, message: isRtl ? 'معرف بيع غير صالح' : 'Invalid sale ID' });
      }
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!isValidObjectId(id)) {
        console.error(`[${new Date().toISOString()}] Delete sale - Invalid ID:`, { id });
        await session.abortTransaction();
        return res.status(400).json({ success: false, message: isRtl ? 'معرف بيع غير صالح' : 'Invalid sale ID' });
      }
