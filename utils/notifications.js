...
const createNotification = async (userId, type, message, data = {}, io) => {
  try {
    ...
    if (targetUser.role === 'production' && targetUser.department?._id) rooms.push(`department-${targetUser.department._id}`);
    if (targetUser.role === 'branch' && targetUser.branch?._id) rooms.push(`branch-${targetUser.branch._id}`);
    if (targetUser.role === 'chef') rooms.push(`chef-${userId}`);
...
const setupNotifications = (io, socket) => {
  ...
  const handleTaskAssigned = async (data) => {
    ...
    const eventData = {
      ...
      data: { orderId, taskId, branchId: order.branch?._id || branchId, chefId, productId, productName, quantity, orderNumber: order.orderNumber, eventId: `${taskId}-new_production_assigned_to_chef` },
      read: false,
      createdAt: new Date().toISOString(),
      sound: 'https://eljoodia-client.vercel.app/sounds/notification.mp3',
      vibrate: [400, 100, 400],
    ...
    const rooms = ['admin', 'production', `chef-${chefId}`, `branch-${order.branch?._id || branchId}`];
    rooms.forEach(room => io.of('/api').to(room).emit('newNotification', eventData));
  ...
  const handleTaskCompleted = async (data) => {
    ...
    const eventData = {
      ...
      data: { orderId, taskId, branchId: order.branch?._id, chefId, productName, orderNumber: order.orderNumber, eventId: `${taskId}-task_completed` },
      read: false,
      createdAt: new Date().toISOString(),
      sound: 'https://eljoodia-client.vercel.app/sounds/notification.mp3',
      vibrate: [200, 100, 200],
    ...
    const rooms = ['admin', 'production', `chef-${chefId}`, `branch-${order.branch?._id}`];
    rooms.forEach(room => io.of('/api').to(room).emit('newNotification', eventData));
  ...
  const handleOrderConfirmed = async (data) => {
    ...
    const order = await Order.findById(orderId).populate('branch', 'name').lean();
    if (!order) {
      console.error(`[${new Date().toISOString()}] Order not found for branchConfirmed: ${orderId}`);
      return;
    }
  ...
  const handleOrderDelivered = async (data) => {
    ...
    const eventData = {
      ...
      data: { orderId, branchId, orderNumber, eventId: `${orderId}-order_delivered` },
      read: false,
      createdAt: new Date().toISOString(),
      sound: 'https://eljoodia-client.vercel.app/sounds/notification.mp3',
      vibrate: [300, 100, 300],
  ...
  const handleReturnStatusUpdated = async (data) => {
    ...
    const order = await Order.findById(orderId).populate('branch', 'name').lean();
    if (!order) {
      console.error(`[${new Date().toISOString()}] Order not found for returnStatusUpdated: ${orderId}`);
      return;
    }
    ...
    const eventData = {
      ...
      data: { returnId, orderId, branchId, orderNumber: order.orderNumber, eventId: `${returnId}-return_status_updated` },
      read: false,
      createdAt: new Date().toISOString(),
      sound: 'https://eljoodia-client.vercel.app/sounds/notification.mp3',
      vibrate: [200, 100, 200],
